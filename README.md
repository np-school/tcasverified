...
| `guidance.html` | ครูแนะแนว | ตรวจสอบขั้นแรกแทนครูกลุ่มสาระเฉพาะกลุ่มสาระ "แนะแนว" + ยืนยันขั้นสุดท้ายทุกกลุ่มสาระ + แก้ไข/ตีกลับรายการนักเรียน + ส่งออก CSV |
| `admin.html` | เจ้าหน้าที่/แอดมิน | ทำเนียบบุคลากร, เพิ่ม-ลบสิทธิ์ครู, ตั้งค่าห้องเรียน/กลุ่มการเรียน/ประเภทกิจกรรม |
...
staff/{email}                → { name, position, department, email, phone, subject, note, updatedAt }
```

**ทำเนียบบุคลากร** จัดการที่แท็บ "บุคลากร" ใน `admin.html` (แอดมินเท่านั้นที่แก้ไขได้ ส่วนอ่านได้ทุกบัญชีในโดเมนโรงเรียน
ตาม `firestore.rules`) เพิ่ม/แก้/ลบทีละคนได้ตรงๆ ในตาราง หรือกดปุ่ม "นำเข้า CSV" เพื่อนำเข้า/อัปเดตทีเดียวจากไฟล์
ที่มีหัวคอลัมน์ `ชื่อ-นามสกุล, ตำแหน่ง, กลุ่มงาน, อีเมล, เบอร์โทร, วิชาที่สอน, หมายเหตุ` (ลำดับคอลัมน์สลับได้ — ระบบยึดหัว
คอลัมน์) โดยใช้อีเมลเป็นรหัสอ้างอิง ถ้าอีเมลตรงกับที่มีอยู่แล้วจะอัปเดตข้อมูลแทนการเพิ่มซ้ำ
...
9.5. นำเข้าทำเนียบบุคลากรทั้งโรงเรียนได้ทันทีที่แท็บ "บุคลากร" → "นำเข้า CSV" โดยเลือกไฟล์ CSV
    ที่มีหัวคอลัมน์ `ชื่อ-นามสกุล, ตำแหน่ง, กลุ่มงาน, อีเมล, เบอร์โทร, วิชาที่สอน, หมายเหตุ`
...
- **role `guidance` ทำหน้าที่ 2 ขั้นตอน**: (1) ตรวจสอบขั้นแรกแทนครูกลุ่มสาระ แต่ **เฉพาะกิจกรรมที่กลุ่มสาระ
  == "แนะแนว"** เท่านั้น (แท็บ "ตรวจขั้นแรก (แนะแนว)" ใน `guidance.html`) และ (2) ยืนยันขั้นสุดท้ายของทุก
  กลุ่มสาระตามปกติ — ชื่อกลุ่มสาระที่ใช้เทียบถูก hardcode ไว้ที่ตัวแปร `GUIDANCE_DEPARTMENT_NAME` ใน
  `js/guidance.js` ถ้าเปลี่ยนชื่อกลุ่มสาระนี้ใน `admin.html` ต้องแก้ค่านี้ให้ตรงกันด้วย ไม่งั้นแท็บนี้จะไม่เจอรายการ
